let input = `p=<2366,784,-597>, v=<-12,-41,50>, a=<-5,1,-2>
...
let particles = input.split('\n').map((line, i) => {
  ...
  return {
    id: i,
    p: match[1].split(',').map(n => parseInt(n)),
    v: match[2].split(',').map(n => parseInt(n)),
    a: match[3].split(',').map(n => parseInt(n))
  }
});

function abs(vector) {
  return vector.reduce((total, value) => total + Math.abs(value), 0);
}

function filterToMinimum(list, property) {
  const minValue = Math.min(...list.map(particle => abs(particle[property])));
  return list.filter(particle => abs(particle[property]) === minValue);
}

const lowestAcceleration = filterToMinimum(particles, 'a');
const lowestVelocity = filterToMinimum(lowestAcceleration, 'v');
const result1 = lowestVelocity[0].id;
result1;
...
while (ticks++ < 50) {
  particles = particles.map(particle => {
    let { id, p, v, a } = particle;
    v = add(v, a);
    p = add(p, v);
    return { id, p, v, a };
  });
  particles = particles.filter(particle => {
    return particles.filter(comparison => {
      return particle.p.toString() === comparison.p.toString() && particle.id !== comparison.id;
    }).length === 0;
  });
}

const result2 = particles.length;
result2;
