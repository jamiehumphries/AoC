...
const valid1 = phrases.filter(phrase => phrase.length === (new Set(phrase)).size);
const result1 = valid1.length;
result1;

const valid2 = phrases.filter(phrase => {
  const sorted = phrase.map(word => word.split('').sort().join(''));
