...
const programs = lines.map(line => line.match(/\w+/)[0]);
const childPrograms = lines.reduce((arr, line) => arr.concat(line.match(/(?<= )\w+/g) || []), []);

const root = programs.filter(prog => childPrograms.indexOf(prog) === -1)[0];
const result1 = root;
result1;
...
lines.forEach(line => {
  const program = line.match(/\w+/)[0];
  weights[program] = parseInt(line.match(/\d+/)[0]);
  children[program] = line.match(/(?<= )\w+/g) || [];
});

const towerWeights = {};
function towerWeight(program) {
  if (towerWeights[program]) {
    return towerWeights[program];
  }
  const programChildren = children[program];
  return weights[program] + programChildren.reduce((total, child) => total + towerWeight(child), 0)
}

const rootWeight = towerWeight(root);

function getCorrectedWeight(parent, weightDifference) {
  const programChildren = children[parent];
  const childWeights = programChildren.map(child => towerWeight(child));
  const badWeight = childWeights.filter((outerValue, outerIndex, arr) => {
    return arr.filter((innerValue, innerIndex) => {
      return innerValue === outerValue && innerIndex !== outerIndex;
    }).length === 0;
  })[0];
  if (!badWeight) {
    return weights[parent] - weightDifference;
  }
  const goodWeight = childWeights.filter(weight => weight !== badWeight)[0];
  const badChild = programChildren[childWeights.indexOf(badWeight)];
  return badChild ? getCorrectedWeight(badChild, badWeight - goodWeight) : parent;
}

const result2 = getCorrectedWeight(root);
result2;
