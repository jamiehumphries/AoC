const input = `g dec 231 if bfx > -10
k dec -567 if wfk == 0
jq inc 880 if a < 2
sh inc -828 if nkr < -5
w inc 595 if nkr > -10
...
const instructions = lines.map(line => {
  const match = line.match(/(\w+) (.*) if (\w+) (.*)/);
  return {
    modifiedRegister: match[1],
    modifier: match[2].replace('inc', '+=').replace('dec', '-='),
    conditionalRegister: match[3],
    condition: match[4]
  };
});
...
instructions.forEach(instruction => {
  const { modifiedRegister, modifier, conditionalRegister, condition } = instruction;
  [modifiedRegister, conditionalRegister].forEach(reg => {
    if (registers[reg] === undefined) {
      registers[reg] = 0;
    }
  });
  const code = `if (registers.${conditionalRegister} ${condition}) {
    registers.${modifiedRegister} ${modifier};
  }`;
  eval(code);
  if (registers[modifiedRegister] > max) {
    max = registers[modifiedRegister];
  }
});

const result1 = Math.max(...Object.values(registers));
result1;
