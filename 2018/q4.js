let input = `[1518-08-29 00:24] falls asleep
...
lines.forEach(line => {
  const guardChange = line.match(/Guard #(\d+) begins shift$/);
  if (guardChange) {
    currentGuard = parseInt(guardChange[1]);
    if (!sleepPatterns[currentGuard]) {
      sleepPatterns[currentGuard] = [...Array(60).keys()].fill(0);
    }
    return;
  }
  const time = parseInt(line.match(/\[1518-\d\d-\d\d \d\d:(\d\d)\]/)[1]);
  if (line.endsWith('falls asleep')) {
    fellAsleepAt = time;
  } else {
    for (let minute = fellAsleepAt; minute < time; minute++) {
      sleepPatterns[currentGuard][minute]++;
    }
  }
});

const guards = Object.keys(sleepPatterns).map(id => parseInt(id));

function totalSleep(id) {
  return sleepPatterns[id].reduce((total, value) => total + value, 0);
}

const sleepiestGuard = guards.sort((g1, g2) => totalSleep(g2) - totalSleep(g1))[0];

function getSleepiestMinute(id) {
  const patterns = sleepPatterns[id];
  const sleepiness = Math.max(...patterns);
  const minute = patterns.indexOf(sleepiness);
  return { sleepiness, minute };
}

console.log(sleepiestGuard * getSleepiestMinute(sleepiestGuard).minute);

let result = null;
let maxSleepiness = 0;
guards.forEach(id => {
  const { sleepiness, minute } = getSleepiestMinute(id);
