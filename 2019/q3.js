...
path1.forEach(move => {
  ...
  for (let d = 1; d <= distance; d++) {
    ...
    steps1++
    const pointKey = pos1.toString()
    points1[pointKey] = points1[pointKey] || steps1
  }
})
...
path2.forEach(move => {
  ...
  for (let d = 1; d <= distance; d++) {
    ...
    steps2++
    const pointKey = pos2.toString()
    points2[pointKey] = points2[pointKey] || steps2
  }
})

const intersection = _.intersection(Object.keys(points1), Object.keys(points2))
const distances = intersection.map(point => point.split(',').reduce((a, b) => a + Math.abs(parseInt(b)), 0))
console.log(Math.min(...distances))
const steps = intersection.map(point => points1[point] + points2[point])
